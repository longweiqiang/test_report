...
def query_indb() :
    ...
    # 获取昨天的上线需求
    story_sql = "select * from commit_report where release_time >= '{0}' and release_time <= '{1}' and tapd_type = 0".format(str(zeroYesterday), str(lastYesterday))
    cursor.execute(story_sql)
    story_results = cursor.fetchall()

    # 获取昨天的计划上线需求
    story_sql = "select * from commit_report where release_time >= '{0}' and release_time <= '{1}' and tapd_type = 0 and is_plan = 1".format(
        str(zeroYesterday), str(lastYesterday))
    cursor.execute(story_sql)
    is_plan = cursor.fetchall()

    # 获取昨天的非计划上线需求
    story_sql = "select * from commit_report where release_time >= '{0}' and release_time <= '{1}' and tapd_type = 0 and is_plan = 0".format(
        str(zeroYesterday), str(lastYesterday))
    cursor.execute(story_sql)
    ...
    '<p><strong>BUG数:<font color="#dd0000"><b>' + str(len(story_results)) + ',已修复' + str(len(story_results)) + '</b></font></strong><br>'\
             '<strong>需求:<font color="#dd0000"><b>' + str(len(story_results)) + '个(计划' + str(len(is_plan)) + '个)' + '个(非计划' + str(len(not_plan)) + '个)' + '</font></strong></p>'\
    ...
    th1 = '<span style="color: rgb(80, 80, 79); font-family: 微软雅黑, sans-serif; font-size: 15px; line-height: normal; background-color: window;"></span><p>' \
    '<strong>线上bug：<font color="#dd0000"><b>' + str(len(bug_results)) + '个(非计划' + str(len(not_plan)) + '个)' + '</font></strong></p>'\
             '<table border="1" cellspacing="0" cellpadding="3" bordercolor="#000000" width="80%" align="center" >' \
                    '<tr bgcolor="#F79646">' \
                        '<th>ID</th>' \
                        '<th>summary</th>' \
    ...
    for row in bug_results:
        ...
        td1 = td1 + '<td>' + '<font color=' + color + '>' + str(tapd_id) + '</font>' + '</td>'
        td1 = td1 + '<td>' + '<font color=' + color + '>' + str(name) + '</font>' + '</td>'
        td1 = td1 + '<td>' + '<font color=' + color + '>' + str(status) + '</font>' + '</td>'
        td1 = td1 + '<td>' + '<font color=' + color + '>' + str(release_time) + '</font>' + '</td>'
        td1 = td1 + '<td>' + '<font color=' + color + '>' + str(environment) + '</font>' + '</td>'
        td1 = td1 + '<td>' + '<font color=' + color + '>' + str(tester) + '</font>' + '</td>'
        td1 = td1 + '<td>' + '<font color=' + color + '>' + str(developer) + '</font>' + '</td>'
        td1 = td1 + '<td>' + '<font color=' + color + '>' + str(project) + '</font>' + '</td>'
        td1 = td1 + '<td>' + '<font color=' + color + '>' + str(comments) + '</font>' + '</td>'
        tr1 = tr1 + '<tr>' + td1 + '</tr>'
    ...
    body = tr
    body1 = tr1
    tail='</table><br></body></html>'
    # 将内容拼接成完整的HTML文档
    mail = header + th + body + tail + header + th1 + body1 + tail
    db.close()
    return mail
